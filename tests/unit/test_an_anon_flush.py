...
единственная причина, по которой аналитика НЕ едет в ``/telemetry/events``
...
на них стоит метрика активаций. Отправь мы их в ``/telemetry/events``, они бы
...
- деградировать некуда (токена и не было) → ретрая нет, конверты остаются
  (без зацикливания);
- залипший ``/telemetry/events`` (нет Bearer) НЕ душит анонимную аналитику.
"""

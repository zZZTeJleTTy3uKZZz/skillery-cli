"""``skills-hub new <slug> --kind prompt|comprehensive|tooling`` — scaffold навыка (E2).
...
- ``tooling`` — то же + ``_skill_meta.toml`` (E6-схема: kind, ``[[cli]]``/``[[mcp]]``,
  ``runtime_dependencies``) + ``--with-cli`` встроенный CLI-скелет на clikit
...
Шаблоны — чистые рендер-функции в ``skills_hub_cli.templates`` (тестируемы
...
def cmd_new(
    ...
    (E6-схема) и, с ``--with-cli``, CLI-скелет на clikit. После генерации

"""``skills-hub event track`` — добавить event в очередь (E23 / E6).

Команда НЕ отправляет event напрямую в backend — пишет в локальную
очередь ``~/.skills-hub/events.queue.json``. Daemon (``skills-hub
...
def register(app: typer.Typer) -> None:
    ...
    event_app = typer.Typer(no_args_is_help=True, help="Event tracking (E23)")

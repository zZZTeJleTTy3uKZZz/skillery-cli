"""E2 — чистые рендер-функции шаблонов навыка (scaffold).

Эталон — ``reverse_factory.factory.skill_emitter``: шаблоны это чистые
строки с ``{{token}}``-подстановкой, рендереры отделены от записи на диск
(она в ``commands/scaffold.py``). Зависимостей вне stdlib нет намеренно
(frontmatter/TOML генерируем сами — лёгкий слой, как у фабрики).
...
- ``render_skill_meta_toml`` — ``_skill_meta.toml`` по E6-схеме
...
def render_skill_meta_toml(
    *,
    ...
    """Сгенерировать ``_skill_meta.toml`` по E6-схеме (kind=tooling).
    ...
    Результат парсится stdlib ``tomllib`` и проходит backend E6-парсер.
    """
    pkg = module_name(slug)
    lines = [

...
def _build_runner(
    *,
    ...
    if reconcile_installs:
        ...
        # Отсчёт «давно ли делали тяжёлый проход» — ОТ СТАРТА ДЕМОНА, а не от нуля:
        # monotonic() — это аптайм машины, и с нулём сравнение означало «машина
        # работает дольше порога». На свежезагруженном ПК демон первые
        # _HEAVY_RECONCILE_SEC секунд аптайма не делал device-sync вовсе, хотя
        # при старте синхронизироваться нужно сразу. Сдвиг назад на порог даёт
        ...
        async def _reconcile() -> None:
            ...
            if now - last_heavy["at"] >= _HEAVY_RECONCILE_SEC:
                ...
                async with reconcile_lock:
                    ...
                    # #1490: набор лизов способностей. Здесь же, а не отдельным
                    # каденсом: это ТОТ ЖЕ фоновый ритм (второй рубеж отзыва по
                    # контракту лиза — «до 180 с»), и второй таймер означал бы
                    # второй ответ на вопрос «как часто демон говорит с хабом».
                    # Сам перевыпуск лизов при этом идёт РЕЖЕ такта: порог —
                    # доля TTL (⅓), то есть ≈3 похода в сутки, а не 480.
                    await main_mod._reconcile_capability_leases(cfg, access)

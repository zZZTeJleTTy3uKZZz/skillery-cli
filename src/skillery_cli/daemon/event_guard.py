"""Анти-спам guard перед публикацией event в общую исходящую очередь.
...
timestamps) персистится в sidecar-JSON ``~/.skillery/events.guard.json``.

Это НЕ очередь и требование «очередь одна» (#1180) не нарушает: здесь лежит
скользящее окно отпечатков, из которого ничего никуда не доставляется, а
устаревшее вычищается при каждом обращении. Guard — свойство ПРОДЮСЕРА (не
класть одинаковое дважды), поэтому переезд хранилища событий в общий outbox
его не затронул.

Две защиты, обе принимают решение ДО публикации конверта:

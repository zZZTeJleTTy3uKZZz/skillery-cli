...
Тело запроса ОБЯЗАНО уходить с заголовком ``Content-Type: application/json``:
``glab`` его не ставит, и GitLab отбивает запрос с ``415``, не читая тело — hook
не создавался вовсе (#1266, см. :func:`_api`).

Секрет НИКОГДА не уходит в argv (в списке процессов его видно всей машине):
тело отдаём провайдерскому CLI через ``--input <файл>`` — временный файл
режимом ``0600``, удаляемый на любом исходе.
...
def _api(
    ...
    *,
    ...
    ПОЧЕМУ ЗДЕСЬ ЯВНЫЙ ``Content-Type`` — ЭТО И ЕСТЬ ДЕФЕКТ #1266. ``glab api``
    тело отправляет, но заголовок НЕ ставит, и GitLab отбивает запрос с
    ``415 "The provided content-type '' is not supported."`` ещё до эндпоинта:
    ``POST .../hooks`` не создавал hook НИКОГДА, а наружу шла невнятная ошибка
    API. Проверено на живом gitlab.com на реальном проекте — протокол проверки
    и три контрольные точки (415 → 400 → 422) описаны у
    :data:`repo_connect.JSON_CONTENT_TYPE_HEADER`. Тот же дефект в #1225 ломал
    создание project-токена (там он накладывался на ``-f "scopes[]=..."``).

    ⚠️ РАНЕЕ ПРИЧИНА БЫЛА ОПРЕДЕЛЕНА НЕВЕРНО, и это стоит помнить, чтобы не
    ходить по кругу: считалось, что ``glab api --input -`` отправляет ПУСТОЕ тело
    (якобы ``Content-Length`` отсутствует). Это НЕ подтвердилось: на живом
    gitlab.com тело доезжает и через stdin, и через файл — сервер возвращает
    ровно то, что мы послали. Прежний «захват» ловил не пустое тело, а
    отсутствие ``Content-Type``: приёмник отвечал 415, не читая тело.

    ТЕЛО ВСЁ РАВНО ОТДАЁТСЯ ФАЙЛОМ, но по СВОЕЙ, честной причине: канал stdin
    для передачи секрета не нужен (``--input <файл>`` работает у обоих
    провайдеров одинаково), а файл проверяем тестом на права. В argv секрет НЕ
    попадает ни в одном из вариантов — в списке процессов его видно всей машине.
    Временный файл создаётся режимом ``0600`` в личном каталоге пользователя и
    удаляется в ``finally`` — в том числе если раннер бросил исключение
    (см. ``repo_connect.write_temp_json_body``).
    ...
    if body is not None:
        ...
        # Заголовок ОБЯЗАТЕЛЕН: без него GitLab отвечает 415, не читая тело.
        args += ["--header", JSON_CONTENT_TYPE_HEADER, "--input", body_file]
    try:
        proc = runner(
            args,
            capture_output=True,
